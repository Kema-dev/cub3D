...
void						ft_cub3d_set_sprite_addr(t_data *data, ssize_t y, int tex_x, int stripe)
{
	int d;
	int	tex_y;

	d = y * 256 - data->map_params->res_height * 128 + data->sprite->height * 128;
	tex_y = ((d * data->texture[4].y) / data->sprite->height) / 256;
	data->addr[y * data->line_length + stripe * data->bits_per_pixel / 8] = data->texture[4].addr[tex_y * data->texture[4].size_line + tex_x * data->texture[4].bits_per_pixel / 8 ];
	data->addr[y * data->line_length + stripe * data->bits_per_pixel / 8 + 1] = data->texture[4].addr[tex_y * data->texture[4].size_line + tex_x * data->texture[4].bits_per_pixel / 8 + 1];
	data->addr[y * data->line_length + stripe * data->bits_per_pixel / 8 + 2] = data->texture[4].addr[tex_y * data->texture[4].size_line + tex_x * data->texture[4].bits_per_pixel / 8 + 2];
}
...
void						ft_cub3d_put_sprites(t_data *data)
{
	...
	while (stripe < (int)data->sprite->end_x)
	{
		...
		if ((data->sprite->trans_y > 0) && (stripe >= 0) && (stripe < (int)data->map_params->res_width) && (data->sprite->trans_y < data->sprite->z_buffer[stripe]))
		{
			...
			while (y < data->sprite->end_y)
			{
				d = y * 256 - data->map_params->res_height * 128 + data->sprite->height * 128;
				tex_y = ((d * data->texture[4].y) / data->sprite->height) / 256;
				if (data->texture[4].addr[tex_y * data->texture[4].size_line + tex_x * data->texture[4].bits_per_pixel / 8])
...
void						ft_cub3d_project_sprites(t_data *data)
{
	...
	while (++i < (ssize_t)data->sprite->count)
	{
		...
		data->sprite->trans_x = data->sprite->inv_det * (data->dir_y * data->sprite->x \
							- data->dir_x * data->sprite->y);
		data->sprite->trans_y = data->sprite->inv_det * (-data->plane_y * data->sprite->x \
							+ data->plane_x * data->sprite->y);
		data->sprite->screen = data->map_params->res_width / 2 \
							* (1 + data->sprite->trans_x / data->sprite->trans_y);
		data->sprite->height = abs((int)(data->map_params->res_height / data->sprite->trans_y));
		data->sprite->start_y = -data->sprite->height / 2 + data->map_params->res_height / 2;
